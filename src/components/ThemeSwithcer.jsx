...
const activeTheme = "flex items-center justify-center w-5 h-5 rounded-full";

const ThemeSwitcher = () => {
  const [theme, setTheme] = useState("light");
  useEffect(() => {
    const localTheme = localStorage.getItem("theme");
    ChangeClickHnadlers(localTheme);
  }, []);
  const ChangeClickHnadlers = (theme) => {
    if (!theme) {
      return;
    }
    if (theme === "dark") {
  ...
  return (
    <div className="flex w-14 h-8 items-center rounded-full justify-around  bg-[#2e2f33] ">
      <button
        onClick={() => ChangeClickHnadlers("light")}
        className={`${activeTheme} ${theme === "light" ? "bg-white" : ""}`}
      >
        <svg
          width="14"
          height="13"
      ...
      <button
        onClick={() => ChangeClickHnadlers("dark")}
        className={`${activeTheme} ${theme === "dark" ? "bg-white" : ""}`}
      >
        <svg
          width="12"
          height="12"
